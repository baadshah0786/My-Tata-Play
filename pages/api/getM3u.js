...
export default async function handler(req, res) {
    ...
    if (uData.tsActive) {
        ...
        res.status(409).json({ error: "Tata Play Deactivated" });
}


import fetch, { Headers } from "cross-fetch";
import starChans from "../../starChans";
const baseUrl = "https://ts-api.videoready.tv";
...
const getUserChanDetails = async (userChannels) => {
    ...
    myHeaders.append("authority", "ts-api.videoready.tv");
    ...
    while (chanIds.length > 0) {
        ...
        await fetch("https://ts-api.videoready.tv/content-detail/pub/api/v1/live-tv-genre/channels?genre=&language=&channelIds=" + chanIdsStr, requestOptions)
            ...
            if (chansList.length > 0) {                
                ...
                for (let i = 0; i < chansList.length; i++) {
                    ...
                    if (chanEnts.length > 0) {
                        ...
                        m3uStr += '#EXTINF:-1 tvg-id=\"tatasky_' + chansList[i].channelMeta.id.toString() + '\" ';                        
